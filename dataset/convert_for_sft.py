...
def explode_episode(
    ...
    sys_prefix = [{"role": "system", "content": f"<r:{reward_flag}>"}] if add_reward_token else []
...
def main():
    ...
    ap.add_argument("--oversample-positive", type=float, default=1.0, help="Oversample factor for episodes with reward=1 (e.g., 3.0).")
    ...
    for ep in episodes:
        all_rows.extend(explode_episode(ep, add_reward_token=not args.no_reward_token))
    ...
    write_jsonl(all_rows, f"{args.out}.jsonl")
